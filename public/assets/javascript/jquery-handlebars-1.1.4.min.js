...
function(a) {
    ...
    function b(a, b, c) {
        return a = a.replace(/[(^\s)(\s$)]/g, ""), a.length ? a + "/" + b + "." + c : b + "." + c
    }

    function c(a) {
        return b(h.templatePath, a, h.templateExtension)
    }

    function d(a) {
        return b(h.partialPath, a, h.partialExtension)
    }

    function e(b, c) {
        a.get(d(b), function(a) {
            Handlebars.registerPartial(c, a)
        }, "text")
    }
    var f = {},
        g = {
        ...
        h = a.extend({}, g);
    a.handlebars = function() {
        if ("string" != typeof arguments[0]) {
            var b = arguments[0];
            if (h = a.extend(g, b), h.templatePath = h.templatePath.replace(/\\\/$/, ""), h.partialPath = h.partialPath.replace(/\\\/$/, ""), b.hasOwnProperty("partials")) {
                var c;
                c = "string" != typeof b.partials ? b.partials : b.partials.split(/\s+/g);
                for (var d = 0; d < c.length; d++) e(c[d], c[d])
            }
        } else switch (arguments[0]) {
            case "partial":
                arguments.length < 3 ? e(arguments[1], arguments[1]) : e(arguments[1], arguments[2]);
                break;
            case "helper":
    ...
    }, a.fn.render = function(b, d) {
        var e = c(b);
        if (f.hasOwnProperty(e)) this.html(f[e](d)).trigger("render.handlebars", [b, d]);
        else {
            var g = this;
            a.get(e, function(a) {
                f[e] = Handlebars.compile(a), g.html(f[e](d)).trigger("render.handlebars", [b, d])
            }, "text")
        }
        return this
    }
